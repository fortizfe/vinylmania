 ...
 * Library-only.
 */
describe('RecordListRow return path (feature 068, US2)', () => {
  ...
  function renderRowWithFrom(entry: EnrichedLibraryEntry, from = LIBRARY_PATH) {
    return render(
      <MemoryRouter initialEntries={['/app/library']}>
        <Routes>
          <Route
            ...
            element={
              <ul>
                <RecordListRow entry={entry} from={from} />
              </ul>
            }
          />
  ...
  it('carries the library path as router state on the row link', async () => {
    renderRowWithFrom(okEntry());
