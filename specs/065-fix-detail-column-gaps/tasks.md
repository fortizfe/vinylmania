...
- [X] T016 [US3] Run `cd e2e && npx playwright test release-detail-responsive record-detail-responsive master-release-detail-responsive` (chromium + webkit) and confirm every test passes. Depends on: T014, T015, and Phase 3/4 implementation being complete. (Surfaced and fixed a real WebKit-only regression from T011's Flexbox restructure — the gallery card collapsed to ~34px tall on WebKit; fixed with an explicit `lg:basis-[30rem]` on the gallery card only, preserving Chromium's existing asymmetric-shrink behavior for the tall-info-stack case. Final local run: 70/70 passing across chromium + webkit.

  Post-merge CI finding #1: the 4 `useIndependentColumnLayout`-based gap-regression tests (2 in `release-detail-responsive.spec.ts`, 2 in `record-detail-responsive.spec.ts`) flaked webkit-only in GitHub Actions CI, despite passing locally every time. Initial fix added `waitForColumnLayoutSettled` (poll `getBoundingClientRect()` until two reads agree) before the existing `boundingBox()` calls — this did NOT fix it.

  Post-merge CI finding #2: CI still failed deterministically (`railGap` exactly `-178` every retry). A second theory — that `Locator.boundingBox()` returned stale geometry — was also wrong: CI failed identically after switching to in-page `getBoundingClientRect()` reads.

  Actual root cause (found by reproducing CI's **Linux** WebKit locally — a Docker Playwright browser server with `exposeNetwork: '<loopback>'`; local macOS WebKit never reproduced it): right after mount, every slot's recorded height was 0 (tops `0/24/48`, container `48px`), because the hook seeded heights **only** from the first `ResizeObserver` notification, which Linux WebKit delivers well over 100ms after first paint. Until then all cards overlap at the top — a real transient UX defect on that engine, not just a test artifact; `-178` is exactly that state (`292 − (268 + 202)`). The "two identical reads" settle-wait was fooled because nothing changes during that window. Fixed in production: `useIndependentColumnLayout` now sets up its observers in a `useLayoutEffect` and seeds every slot's height synchronously via `getBoundingClientRect()` before first paint, keeping `ResizeObserver` only for later size changes. Test-first: added a hook unit test with an observer that never fires, confirmed red (`24px` vs `344px`), then green. Verified: 4/4 previously failing tests and 35/35 (×2 runs) webkit specs on Linux WebKit, 70/70 chromium+macOS-webkit responsive specs, 132/132 other chromium specs touching the detail pages, 816/816 frontend unit tests.)
