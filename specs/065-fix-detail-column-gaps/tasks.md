...
- [X] T016 [US3] Run `cd e2e && npx playwright test release-detail-responsive record-detail-responsive master-release-detail-responsive` (chromium + webkit) and confirm every test passes. Depends on: T014, T015, and Phase 3/4 implementation being complete. (Surfaced and fixed a real WebKit-only regression from T011's Flexbox restructure — the gallery card collapsed to ~34px tall on WebKit; fixed with an explicit `lg:basis-[30rem]` on the gallery card only, preserving Chromium's existing asymmetric-shrink behavior for the tall-info-stack case. Final local run: 70/70 passing across chromium + webkit.
  ...
  Post-merge CI finding #2: with the settle-wait in place, CI still failed deterministically (`railGap` exactly `-178` on every retry, never flaky-random) on the same 4 tests. Downloaded the CI run's trace/screenshot artifacts: the failure screenshot showed the layout was actually correct and gap-free at the moment of failure — `Locator.boundingBox()` (a separate CDP round-trip) was returning stale/incorrect geometry for these `position: absolute` cards on webkit, while `page.evaluate(() => el.getBoundingClientRect())` (in-page JS, forces a synchronous layout flush) consistently read the correct values. Fixed by replacing `boundingBox()` entirely with a new `waitForStableCardGeometry` helper in `e2e/helpers/columnLayout.ts` that reads geometry via `getBoundingClientRect()` from within the page and returns the already-settled result — no separate follow-up `boundingBox()` call remains anywhere in these 4 tests. Re-verified: 3 consecutive full local webkit runs (35/35 each) plus a combined chromium+webkit run (70/70) all green after the fix. No production code changed for either finding.)
