...
- [X] T016 [US3] Run `cd e2e && npx playwright test release-detail-responsive record-detail-responsive master-release-detail-responsive` (chromium + webkit) and confirm every test passes. Depends on: T014, T015, and Phase 3/4 implementation being complete. (Surfaced and fixed a real WebKit-only regression from T011's Flexbox restructure — the gallery card collapsed to ~34px tall on WebKit; fixed with an explicit `lg:basis-[30rem]` on the gallery card only, preserving Chromium's existing asymmetric-shrink behavior for the tall-info-stack case. Final local run: 70/70 passing across chromium + webkit.
  ...
  Post-merge CI finding: the 4 `useIndependentColumnLayout`-based gap-regression tests (2 in `release-detail-responsive.spec.ts`, 2 in `record-detail-responsive.spec.ts`) flaked webkit-only in GitHub Actions CI, despite passing locally every time. Root cause: those tests read `boundingBox()` immediately after the page's first visible content, racing the hook's asynchronous `ResizeObserver` measurement pass — usually won on fast/idle hardware, lost intermittently under CI's slower/more congested webkit execution. Fixed test-side only (no production code change) with a new `e2e/helpers/columnLayout.ts::waitForColumnLayoutSettled` helper, following the existing `settleEntrance.ts` "poll until two consecutive reads agree" pattern, called before measuring in all 4 tests. Re-verified 70/70 passing locally after the fix.)
