...
6. **Material contrast**: `helpers/contrast.ts` checks the toolbar text/border pairings over a fixture cover (a solid black PNG, then a white one) against the D17 floors. Under `page.emulateMedia({ contrast: 'more' })`, the toolbar's computed `backdrop-filter` is `none` and it has a 1 px border. Playwright cannot emulate reduced transparency or missing `backdrop-filter` support, so those two fallbacks are checked manually (§4).
