...
- [X] T068 [P] [US4] `e2e/tests/sheet-drag-dismiss.spec.ts` — synthetic pointer drag on the drawer: 1:1 tracking from grab point; release <45% & slow → springs back; release >45% or ≥500 px/s → closes + fires `onClose`; drag starting mid-scroll (not at boundary) scrolls instead
- [X] T069 [P] [US4] `e2e/tests/gallery-swipe.spec.ts` — horizontal swipe advances image; `aria-current` thumbnail follows; `ArrowLeft/Right` and thumbnail buttons still work; hard flick does not skip two images
...
- [X] T076 [US4] Extend `e2e/tests/header-responsive-nav.spec.ts` with the drawer gesture + parity assertions
- [X] T077 [US4] Run US4 validation (quickstart §US4, incl. real-device manual checks); update US4 rows in [audit.md](./audit.md)
...
- [X] T086 [P] [US5] Gentle opacity-only entrance (token, no movement) for status feedback. **Scoped in T097** to `frontend/src/components/FeedSourceStatusBanner.tsx` only — it mounts when a news source starts failing, so a gentle entrance is genuine status feedback (apple-design §16). Removed from the first-paint static placeholders `frontend/src/components/UnderConstruction.tsx`, `frontend/src/components/LibraryLinkRequired.tsx`, `frontend/src/components/DiscogsRelinkNotice.tsx` (the entrance added no signal there and its ancestor `opacity` ramp was folded into axe-core's contrast maths mid-fade → false-positive `color-contrast` violation).
...
- [X] T091 [P] Add `e2e/tests/motion-performance.spec.ts` — CDP `Emulation.setCPUThrottlingRate(4)`, measure frame interval during modal / drawer / gallery / CollapsibleFilterPanel transitions via rAF + `long-animation-frame` `PerformanceObserver`; asserts the animation window holds a 60 fps median with no sustained stutter and no input-blocking frame (SC-010). Reviewed + run green (T093): steady-state p50 Modal 14.5 / Drawer 14.2 / Gallery 16 / CollapsibleFilterPanel 12.9 ms; longest sub-36fps run ≤ 2; max frame ≤ 59 ms (< 150 ms input ceiling). Whole-arc p95/p99 (mount included) 26–59 ms — recorded as a test annotation + carried to the PR as a follow-up (overlay-mount + first `backdrop-filter` composite spike). Chromium only. Helper additions in `e2e/helpers/motion.ts` (`startFrameCadenceRecorder`, `collectFrameCadence`, `percentile`).
- [X] T092 Run full `frontend` gate: `npm run test` + `npm run lint` + `npm run build`
- [X] T093 Run full `e2e` gate — 324 tests. First run: 320 passed / 2 failed / 2 skipped. Both failures were the axe-core-folds-ancestor-`opacity` false positive on feature-059 entrance animations, fixed test-side like US3's `settleOverlay`: (1) the `status-fade-in` regression (T086) — resolved by scoping the entrance to `FeedSourceStatusBanner` and dropping it from the three first-paint placeholders (see T097); (2) `library-filters.spec.ts:410` / `search-result-filters` axe scans hitting the US2 `CollapsibleFilterPanel` disclosure opacity ramp — resolved with `settleEntranceOpacity(page, '[data-testid="collapsible-filter-body"]')` before `runAxeScan`. Also cleared a non-feature-059 flake: `discogs-catalog-relink.spec.ts:75` (spec 053) fails on a warm shared-Redis `discogs:release:1` entry — cold-cache green (5/5), pre-existing e2e isolation gap flagged for the spec-053 owner. Re-run after fixes: **324 tests → 322 passed / 0 failed / 2 skipped** (the 2 skipped are the pre-existing `test.fixme` at `search-result-filters.spec.ts:442` + `:951`, spec 042). `dark-mode-contrast` + `overlay-contrast` + `motion-performance` (4/4) green on both themes; Modal focus/scroll/restore/backdrop gap closure verified green.
...
- [X] T096 PR description written to [`PR.md`](./PR.md) — the 3 constitution deviations (`motion` dep, `frontend/src/motion/` module, CSS motion custom properties) + the 4 in-flight accepted deviations, known limitations, process notes, verification, and the T094/T095 manual checklist. No `CHANGELOG.md` / `version` edits.
- [X] T097 Full `quickstart.md` pass — DoD table walked (SC-001…SC-010). SC-001/002/003/004/005/006/007/008/010 **PASS**. SC-005 now passes: the `status-fade-in` entrance is scoped to `FeedSourceStatusBanner` (removed from `UnderConstruction` / `LibraryLinkRequired` / `DiscogsRelinkNotice`), its axe scan gated by `settleStatusFadeIn`, and the same settle applied to the US2 filter-disclosure axe scans; `statusMessageEntrance.test.tsx` narrowed accordingly. SC-010 PASS with a documented one-time mount-spike finding (Deviation #8). SC-009 (human A/B survey) deferred to T095; SC-010 real-device portion + research "Open feel-checks" deferred to T094 (human). Full table in the QA report.
