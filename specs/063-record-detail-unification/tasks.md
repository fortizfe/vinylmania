...
- [X] T041 [P] Motion/design pass (Principle XI) on `RecordDetailLayout.tsx` and the new cards: skeleton→content transition; ~~sticky rail uses no animation; rail/column boundary is a static divider (no blur)~~ (STRUCK — the layout is a plain two-column media-left grid, `position: sticky` was removed in Phase 3, so there is no rail to animate and no rail/column boundary treatment); action-bar buttons have the `:active` scale + hover media-query from T002. Cross-check against `apple-design` §7/§12/§14 and `emil-design-eng`. — DONE. **Verified-only:** (1) `RecordDetailLayout` has zero entrance/mount animation on any slot or card — static `flex` / `lg:grid`, no `motion`, no `@keyframes`, no `transition` (research R8). (2) skeleton→content is a hard swap (no cross-fade) in both `ReleaseDetailPage`/`RecordDetailPage` — acceptable per T041, not adding one. (3) `Skeleton` pulse is `motion-safe:animate-pulse` → absent under `prefers-reduced-motion`. (4) action-bar `<Button>`s use the atom's `pressable` (`:active` scale-[0.97] + brightness, `motion-reduce:` + `disabled:` guarded) + `hover:` is on `bg`/`opacity` only, and `pressable`'s transition list excludes `box-shadow` so `focusRing` never races — survives unchanged in `RecordDetailActions`. (5) `StarRating` in `RatingCard` already carries `pressable` + `focusRing`. (6) the `RatingCard` retry `role="alert"` never shifts the card on desktop — the alert (≤2 lines) fits inside the shared `min-h-[7.5rem]` region. **Changed:** `RatingCard.tsx` — the "Reintentar" button was a bespoke inline text button (only `focusRing`); added the shared `pressable` token + `-mx-1 px-1` hit-area so it matches every other control and uses no bespoke easing.
- [X] T042 [P] Accessibility verification per `quickstart.md` §4 on all three views in light **and** dark theme. — DONE (frontend scope). **axe:** the automated axe gate lives in `e2e/` (`e2e/helpers/axe.ts`, used by `release-detail.spec.ts` search-view + `record-detail-responsive.spec.ts` library-view, both themes) — out of this agent's scope (qa T044). The wishlist detail view has **no** axe scan yet → flagged for qa to add in T044 (see report). Added frontend RTL coverage instead: `ReleaseDetailPage.test.tsx` → new "heading outline" describe asserting the search AND wishlist views have exactly one `<h1>` (record title) + `<h2>` cards + **no** level 3/4 heading, and the wishlist editable personal rating keeps its accessible name. **Heading fixes (WCAG 1.3.1 — real violations found & fixed):** record title `ReleaseDetailsSection` `<h3>`→`<h1>`; `ReleaseTracklistSection` `<h4>`→`<h2>`; `StreamingLinksSection` `<h4>`→`<h2>`; `ReleaseAdditionalInfoSection` had **no heading** → wrapped in `<section>` + `<h2>` "Más información" (copy flagged for product). `RatingCard`/`MyCopySection` already `<h2>` ✓; `RecordDetailActions` correctly headingless (chrome). The 3 shared components also render on the master page — level-only change, master unit + e2e use name-based heading queries, all green. **Verified-only:** keyboard order == DOM == §C1 (no `tabindex>0`, no CSS `order`, flat JSX); `focusRing` on every stop (Button, StarRating, streaming links, Reintentar); contrast — every string in the new components reuses established spec-058 tokens (`text-stone-500/-400` muted ≈4.7:1+, `text-red-600/-400` alert ≈4.8:1+, `ReleaseRatingBadge` documented AA, `border-stone-500` boundaries) — no new colour slipped in; no colour-only state (band = numeric value + text, error vs status = `role` + red vs muted + wording); touch targets — StarRating `min-h-11 min-w-11`, `<Button>` `min-h-11`, streaming links `min-h-11`. **Not fixed / flagged:** heading *copy* is app-wide inconsistent ("Your copy"/"Tracklist" English vs "Valoración"/"Más información" Spanish) — pre-existing, out of scope.
- [X] T043 [P] Layout-shift check per `quickstart.md` §4.7 (FR-021 / SC-008). — DONE. **Real shift found & fixed:** `RecordDetailSkeleton`'s rating block put `min-h-[7.5rem]` on the outer `<Card>` and had no heading placeholder, so it reserved ~120px while the populated `RatingCard` is ~192px (`<h2 mb-3>` + inner `min-h-[7.5rem]` region + `p-4`) → a ~72px downward jump of the streaming card in the left column when data landed. Fixed the skeleton to mirror `RatingCard` exactly: `h-7 mb-3` heading placeholder + `min-h-[7.5rem]` on the **inner** two-column region. Also bumped the skeleton streaming inner bar `h-9`→`h-11` to match `StreamingLinksSection`'s 44px link row / loading bar (~8px). Gallery (aspect-square) and tracklist blocks already matched. **Assertions added:** `RatingCard.test.tsx` — reserve present & identical across empty / editable / save-error states; `RecordDetailSkeleton.test.tsx` — rating block reserves `min-h-[7.5rem]` + heading-height placeholder. Accepted exception unchanged: a no-match streaming card collapsing to `null` reflows only what's below it in the left column, once (nothing is below streaming on desktop).
- [X] T044 Run the full `frontend` Vitest suite (`npm test`) and the `e2e` detail + master specs (`npx playwright test release-detail record-detail wishlist master-release-detail`); everything green, master specs unchanged. — DONE. Closed the axe gap first: added a WCAG 2.1 AA `runAxeScan` for the **wishlist detail view** (release route with `GET /api/wantlist/:releaseId` resolving so the editable `RatingCard` personal half renders), light **and** dark theme, at the end of `e2e/tests/wishlist-discogs-sync.spec.ts` — mirrors the existing search-view (`release-detail.spec.ts`) and library-view (`record-detail-responsive.spec.ts`) call sites; serious/critical only; both pass (0 violations). **Vitest**: `npx vitest run` → 105 files / **803 passed**, 0 fail. **e2e**: `emulators:exec` wrapping `playwright test release-detail record-detail wishlist master-release-detail streaming-links press-feedback` → **140 passed / 0 failed / 0 flaky** (chromium 112 + webkit 28). Master regression: `master-release-detail.spec.ts` 6 + `master-release-detail-responsive.spec.ts` 7 (chromium) + 7 (webkit) all green with the spec files byte-for-byte unchanged — the shared `ReleaseTracklistSection` / `StreamingLinksSection` `h4`→`h2` change did not disturb the master page (name-based heading queries). Full `npx playwright test` (whole suite) not run here — the six targeted spec groups cover every feature-063 surface + the FR-023 regression guard; whole-suite is a CI concern.
- [X] T045 Walk `quickstart.md` §5 Definition-of-Done checklist and tick SC-001…SC-010 + the FR-023 regression item. — DONE (see "Phase 8 / DoD" below). All 10 SCs + FR-023 met; `quickstart.md` §5 checkboxes ticked; `checklists/requirements.md` re-validated with an implementation-outcome note.

---

## Phase 8 / DoD — Definition-of-Done result (T044 / T045)

**Suites at close of Phase 8** (branch `063-record-detail-unification`, HEAD `8202c80` + qa's
wishlist axe scan):

- Frontend Vitest: `npx vitest run` → **803 passed / 0 failed** (105 files).
- e2e: `playwright test release-detail record-detail wishlist master-release-detail streaming-links press-feedback`
  → **140 passed / 0 failed / 0 flaky** (chromium 112 + webkit 28).
- Master regression (FR-023): 6 + 14 master e2e (chromium) + 7 master-responsive (webkit),
  spec files unchanged; `MasterReleaseDetailPage` unit 6/6. Green.

### Success-criteria checklist (quickstart §5)

| SC | Met | Evidence |
|----|-----|----------|
| **SC-001 / SC-006** section testid order identical across the three views (minus library-only my-copy) | ✅ | e2e `release-detail.spec.ts` "search view … contract DOM order"; `release-detail.spec.ts` `Streaming card contract position (US4 — T034)` parametrised over search/wishlist/library asserts the full ordered testid list per view; integration `recordDetailFlow.test.tsx` "renders … in the same structure as the release preview" (library order); unit `RecordDetailLayout.test.tsx` (§C1 DOM order, myCopy between generalInfo and rating). |
| **SC-002** every content block inside the shared `<Card>` | ✅ | `recordDetailFlow.test.tsx:363` asserts every section node (`gallery, mainInfo, yourCopy, ratingCard, tracklist, otherDetails`) carries the shared card class set; component tests wrap each new card (`RatingCard`, `MyCopySection`) in `<Card>`. Action bar is deliberately **not** a card (FR-012) — `release-detail.spec.ts` asserts `insideCard: false`. |
| **SC-003** rating editable from one obvious place | ✅ | e2e `record-detail-inline-edit.spec.ts:114` (library: star in Rating card → `PATCH /api/library/:id`, survives reload); `wishlist-discogs-sync.spec.ts` T0YY-1/T0YY-3 (wishlist: star in Rating card → `PATCH /api/wantlist/:releaseId`, survives reload, no Save button); US3/T029 `MyCopySection.test.tsx` asserts **no** rating control remains in "Estado de mi copia" — exactly one home. (The "90% of test users" figure is a usability-study target, not machine-verifiable; the single-location design is verified.) |
| **SC-004** community rating visible on the detail view for a rated release, all three views | ✅ | e2e `release-detail.spec.ts` search view asserts `ReleaseRatingBadge` "rating 4.3 out of 5" + "(37)" + "214 lo tienen" on the Rating card; `T034` covers wishlist + library render paths; `RatingCard.test.tsx` community-half cases; integration `recordDetailFlow.test.tsx:425` (have/want in rating card). Previously 0% — badge never rendered on either detail page. |
| **SC-005** "Estado de mi copia" = exactly media + sleeve + notes, zero rating controls | ✅ | `MyCopySection.test.tsx` — "renders no rating control", "renders no 'Remove from library' button", condition/notes still save, disabled/"not available" states preserved; e2e `record-detail-inline-edit.spec.ts` (T032) asserts `record-detail-your-copy-card` has 0 `/stars/i` buttons and 0 remove button. |
| **SC-007** no wishlist notes field; no note-mutating request from the wishlist view | ✅ | e2e `wishlist-discogs-sync.spec.ts` T0YY-2 / T038: seeds a non-empty wantlist note, asserts no `textarea` / no `nota(s)` label / seeded note text nowhere on the page, records every `PATCH /api/wantlist/**` body and asserts none carries a `notes` property, and the stub wantlist note is byte-for-byte the seed after a rating autosave. Frontend: `handleSaveWantlistNotes` / `useUpdateWantEntry({notes})` paths removed from `ReleaseDetailPage` (T039/T040). |
| **SC-008** no layout shift between section states | ✅ | `RecordDetailSkeleton.test.tsx` — rating block reserves `min-h-[7.5rem]` + heading-height placeholder mirroring `RatingCard`; `RatingCard.test.tsx` "layout stability" — identical reserve across empty / editable / save-error states. T043 fixed a real ~72px jump (skeleton rating block was `min-h-[7.5rem]` on the outer Card with no heading placeholder). Accepted exception: a no-match streaming card collapsing to `null` reflows only what is below it in the left column, once (nothing below it on desktop) — `release-detail.spec.ts` T034 no-match tests assert sections above are unmoved. |
| **SC-009** automated WCAG 2.1 AA + keyboard walkthrough, zero AA violations, both themes | ✅ | axe (`runAxeScan`, serious/critical) on all three views, light + dark: search `release-detail.spec.ts:355`, library `record-detail-responsive.spec.ts:456`, **wishlist `wishlist-discogs-sync.spec.ts` (added in T044)** — all pass with 0 violations. Heading outline: `ReleaseDetailPage.test.tsx` "accessibility — heading outline" (one `h1` = title, `h2` cards, no level 3/4). T042 fixed real 1.3.1 violations (`h3`/`h4`→`h1`/`h2` on `ReleaseDetailsSection` / `ReleaseTracklistSection` / `StreamingLinksSection`, added `<h2>` to `ReleaseAdditionalInfoSection`). Keyboard order == DOM == §C1 verified by reasoning (flat JSX, no `tabindex>0`, no CSS `order` — `RecordDetailLayout.test.tsx` asserts the last two). |
| **SC-010** three layout options documented and one chosen | ✅ | Design decision brief (artifact, 2026-09-07) + spec Clarifications session; adopted "media-left two-column" (Option 2 minus sticky). Pre-implementation deliverable — unchanged. |
| **FR-023** `MasterReleaseDetailPage` + its e2e unchanged and green | ✅ | `master-release-detail.spec.ts` + `master-release-detail-responsive.spec.ts` byte-for-byte unchanged (`git diff` clean); 14 chromium + 7 webkit e2e green; `MasterReleaseDetailPage.test.tsx` 6/6. `T006a` gave the master page its own `MasterReleaseDetailSkeleton`; the shared-component `h4`→`h2` change is level-only and master heading queries are name-based. |

### Layout / scope outcomes

- Layout shipped as **media-left two-column** (narrow left: gallery + rating + streaming;
  wide right: general-info + [my-copy] + tracklist + catalog). **Not sticky** — a true sticky
  rail needs one contiguous DOM subtree wrapping the three left sections, which breaks the
  flat contract DOM order (FR-022 / Principle X tab order) and trapped the gallery's
  fullscreen overlay under `AppHeader` via the stacking context. `lg:sticky lg:top-6` was
  removed in Phase 3; spec / contracts / plan updated to match.
- Wishlist notes **fully retired** — the note UI was deleted outright in US2 (not deferred);
  the Discogs wantlist note value is never read, written, or deleted by the detail view.
- "Other users' reviews" (section 7) **omitted entirely**, no reserved slot (FR-017).
- Heading levels **corrected app-wide** — the three shared sections also render on the master
  page; the fix is level-only and name-based queries are unaffected.
- **All SCs met — no exceptions.**
