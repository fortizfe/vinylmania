...
**Check for extension hooks (before implementation)** — run before delegating to
`/speckit-agent-assign-execute`, using the exact same `before_implement` key and
dispatch rules `/speckit-implement` uses, so installed extensions cannot tell the
...
- [ ] `before_implement` hooks dispatched or skipped per the Pre-Execution Checks above
- [ ] `/speckit-agent-assign-execute` ran to completion (or this command
      stopped early and relayed why, without running the post-execution hooks)
- [ ] `after_implement` hooks dispatched or skipped per the rules in the outline
